    ...
    <>
      <div className="flex h-screen overflow-hidden bg-plexus-surface-subtle">
        <aside className="flex w-[240px] shrink-0 flex-col border-r border-plexus-border bg-plexus-sidebar">
          <div className="titlebar-drag flex h-11 items-center gap-2 border-b border-plexus-border px-4">
            ...
            >
              <div className="flex h-5 w-5 items-center justify-center rounded bg-plexus-brand text-[10px] font-bold text-white">
                P
              </div>
              Plexus
            </button>
              ...
              <div className="mt-1">
                ...
                {projects.map((p) => (
                  <div key={p.id}>
                    <NavItem
                      icon={
                        <div className="flex h-4 w-4 items-center justify-center rounded bg-plexus-brand/15 text-[9px] font-bold text-plexus-brand">
                          {p.key[0]}
                        </div>
                      }
                      label={p.name}
                    ...
                    {p.key === projectKey && (
                      <div className="ml-5 mt-0.5 space-y-0.5 border-l border-plexus-border pl-2">
                        ...
                        <NavItem
                          ...
                          onClick={() => navigate(routes.projectBacklog(orgSlug, p.key))}
                        />
                        <NavItem
                          ...
                          onClick={() => navigate(routes.projectSettings(orgSlug, p.key))}
                        />
                      </div>
                    )}
                  </div>
        ...
        <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
          <header className="flex shrink-0 items-center gap-3 border-b border-plexus-border bg-plexus-surface px-4 py-2">
            {orgSlug ? (
              <div className="min-w-0 flex-1 max-w-lg">
                <IssueSearch
                  ...
                  variant="panel"
                  onIssueSelect={(hit, key) => navigate(routes.issue(orgSlug, key, hit.number))}
                />
              </div>
            ) : (
      ...
      {canCreate && (
        <CreateIssueDialog
          ...
          orgSlug={orgSlug!}
          projectKey={projectKey!}
          onCreated={(issue) => navigate(routes.issue(orgSlug!, projectKey!, issue.number))}
        />
      )}
    </>
  )
  ...
  return (
    <button
      ...
      className={[
        ...
        active
          ? 'bg-plexus-brand/15 font-medium text-plexus-brand'
