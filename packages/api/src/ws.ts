...
function wsBaseUrl(): string {
  const httpBase = getApiConfig().baseUrl.replace(/\/api\/v1\/?$/, '')
...
export function isSprintEvent(type: string): boolean {
  return type === 'sprint.updated'
}

export function isCommentEvent(type: string): boolean {
